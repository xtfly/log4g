...
func TestFormat(t *testing.T) {
	f, _ := NewTextFormatter(CfgFormat{"type": "text", "name": "f1",
...
func TestFormat2(t *testing.T) {
	...
	op := NewBaseOutput(&buf, All)
	f, _ := NewTextFormatter(CfgFormat{"type": "text", "name": "f1",
		"layout": "%{shortfile}"})
	op.SetFormatter(f)
	log := GetLogger("test_format")
	log.SetOutputs([]Output{op})
...
func realFunc(log Logger) {
	log.Debug("xx")
}

type structFunc struct{}

func (structFunc) Log(log Logger) {
	log.Debug("xx")
}
